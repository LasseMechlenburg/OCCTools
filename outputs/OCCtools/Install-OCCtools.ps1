...
try {
 ...
 foreach ($name in @('dist','server.mjs','templates.mjs','logs.mjs','daily-log.mjs','daily-log-request.ps1','log-create.mjs','log-create-request.ps1','checklist-item.mjs','checklist-item-request.ps1','runitems.mjs','checklist-start.mjs','checklist-done.mjs','documents.mjs','pdf-index.py','pdf-preview.py','flow-request.ps1','start-request.ps1','done-request.ps1','init-admin.mjs','seed.json','package.json','README.md','DEPLOY.md')) { Copy-Item -LiteralPath (Join-Path $PSScriptRoot $name) -Destination $targetFull -Recurse }
