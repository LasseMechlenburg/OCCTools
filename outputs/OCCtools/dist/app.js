...
// Visibility is presentation only, not an access-control boundary.
function activeCategories(source=data){
 return source.categories.filter(g=>g.active!==false).map(g=>({...g,tools:g.tools.filter(t=>t.active!==false)}));
}
function activeControl(item,label){
...
function renderEmergency(){
 $('#emergencyShortcut')?.remove();const t=activeCategories().flatMap(g=>g.tools).find(t=>t.id==='emergency');if(!t?.url)return;
...
function startWheel(group){
 ...
 const tools=group?group.tools.filter(t=>t.id!=='emergency'):['occ-calendar','adhoc-calendar','checklist'].map(id=>activeCategories().flatMap(g=>g.tools).find(t=>t.id===id)).filter(Boolean),name=group?.name||'Start vagten',panelId='wheel-'+(group?.id||'start');
...
function render(){activeCategories().forEach(g=>g.tools.forEach(t=>{if(t.id==='runitems')t.name='Åbne checklister';}));document.title=new URLSearchParams(location.search).get('dailyLog')==='1'?'Daily log · OCC':data.title;const p=$('#portal');p.replaceChildren();const title=el('div','pageTitle');const left=el('div');left.append(el('h1','',data.title),el('p','','Dine værktøjer og nye tiltag i OCC.'));title.append(left,el('span','eyebrow','Fælles overblik · daglig operation'));p.append(title);
title.lastElementChild.remove();
p.append(portalSearch());
const projects=data.projects.filter(x=>x.visible);
const head=el('div','sectionHead');head.append(el('h2','','Værktøjer'));p.append(head);const nav=el('nav','wheelCategories');nav.setAttribute('aria-label','Værktøjskategorier');activeCategories().filter(g=>g.id!=='flightpoint'||g.tools.some(t=>t.id==='flightpoint-home')).forEach(g=>nav.append(startWheel(g)));renderEmergency();p.append(nav);document.dispatchEvent(new Event('portalrender'));if(data.projectsVisible&&projects.length)p.append(projectTimeline(projects));p.hidden=false;$('#loading').hidden=true;}
...
function picker(label,items,selected,onchange){const f=field(label,'','select');items.forEach(x=>{const o=el('option','',x.name+(x.active===false?' (inaktiv)':''));o.value=x.id;f.input.append(o);});f.input.value=selected||'';f.input.onchange=()=>onchange(f.input.value);return f.label;}
...
area.append(activeControl(g,'Aktiv menu – vis på forsiden og i søgning'),el('p','muted','En inaktiv menu skjuler alle dens links. Indholdet bevares i admin.'));area.append(field('Menunavn',g.name,'text',v=>g.name=v).label,field('Forklaring til menu',g.description,'textarea',v=>g.description=v).label,field('Fælles how-to / guideline-link',g.manual,'url',v=>g.manual=v).label);const icons=field('Ikon','','select');Object.keys(paths).forEach((key,i)=>{const o=el('option','',['Hotel','Roster','Fly','Manual','Kalender'][i]);o.value=key;icons.input.append(o);});icons.input.value=g.icon;icons.input.onchange=()=>g.icon=icons.input.value;area.append(icons.label);const ga=el('div','actions');ga.append(button('Flyt menu op','quiet',()=>move(draft.categories,g,-1)),button('Flyt menu ned','quiet',()=>move(draft.categories,g,1)),button('Slet menu','quiet danger',()=>{if(confirm(`Slet ${g.name} og dens ${g.tools.length} links?`)){draft.categories=draft.categories.filter(x=>x!==g);renderEditor();}}));area.append(ga,el('h3','','Værktøjer i menuen'));
if(!g.tools.some(t=>t.id===selectedTool))selectedTool=g.tools[0]?.id;area.append(picker('Værktøj',g.tools,selectedTool,v=>{selectedTool=v;renderEditor();}),button('+ Nyt link','quiet',()=>{const t={id:uid(),name:'Nyt værktøj',url:'',description:'',manual:''};g.tools.push(t);selectedTool=t.id;renderEditor();}));const t=g.tools.find(x=>x.id===selectedTool);if(!t)return;area.append(activeControl(t,'Aktivt link – vis på forsiden og i søgning'));if(g.active===false)area.append(el('p','notice','Menuen er inaktiv. Linket vises først, når menuen også er aktiv.'));area.append(field('Navn',t.name,'text',v=>t.name=v).label,field('Adresse (http:// eller https://)',t.url,'url',v=>t.url=v).label,field('Hvad kan man i værktøjet?',t.description,'textarea',v=>t.description=v).label,field('How-to / guideline-link',t.manual,'url',v=>t.manual=v).label,picker('Flyt værktøjet til menu',draft.categories,g.id,v=>{if(v===g.id)return;g.tools=g.tools.filter(x=>x!==t);draft.categories.find(x=>x.id===v).tools.push(t);selectedGroup=v;renderEditor();}));area.append(picker('Åbn som',[{id:'link',name:'Link i nyt vindue'},{id:'pdf',name:'PDF i fast panel'}],t.display||'link',v=>{t.display=v;}));const ta=el('div','actions');ta.append(button('Flyt link op','quiet',()=>move(g.tools,t,-1)),button('Flyt link ned','quiet',()=>move(g.tools,t,1)),button('Slet link','quiet danger',()=>{if(confirm(`Slet ${t.name}?`)){g.tools=g.tools.filter(x=>x!==t);renderEditor();}}));area.append(ta);}
...
function renderCalendarQuickView(){const c=$('#calendarFloatContent');if(!c)return;calendarSnapshot=mergedCalendar();c.replaceChildren(el('h2','','Kalender · tre dage'));const tabs=el('div','editorNav');const refresh=button(flightpointLoading?'Henter…':'Refresh','quiet',refreshFlightpoint);refresh.disabled=flightpointLoading;tabs.append(refresh);c.append(tabs);if(flightpointError)c.append(el('p','error',flightpointError+' Tidligere data er ikke opdateret.'));if(!calendarSnapshot){c.append(el('p','notice',flightpointLoading?'Henter friske kalenderdata…':'Klik på Refresh for at hente de tre dage.'));return;}
 ...
 c.append(el('p','notice',snap.live?`Hentet fra FlightPoint ${stamp}. Klik på Refresh for at hente igen.`:`Testudtræk · hentet ${stamp}. Ingen automatisk opdatering.`));if(snap.partial)c.append(el('p','error','Udtrækket indeholder flere sider. Oversigten kan mangle poster.'));
...
function portalSearch(){
 ...
 function search(){
  ...
  activeCategories().forEach(g=>g.tools.forEach(t=>{if(match([g.name,t.name,t.url,t.description]))found.push({title:t.name,group:searchToolGroup(t),kind:'Menu: '+g.name,tool:t,action:()=>{delete $('#detailContent').dataset.contacts;toolInfo(t);}});}));
...
api('content').then(v=>{data=v;render();if(checklistPopoutId){document.body.classList.add('checklistPopout');openChecklistStandalone(true);}}).catch(e=>{const l=$('#loading');l.replaceChildren(el('p','error','Portalen kunne ikke indlæses. '+e.message),button('Prøv igen','quiet',()=>location.reload()));});
...
function renderPasswordsView(){
 ...
 const refresh=button(flightpointLoading?'Henter…':'Refresh','quiet',refreshFlightpoint);refresh.disabled=flightpointLoading;c.append(refresh);
...
const checklistPopoutParam=new URLSearchParams(location.search).get('checklistRun');
const checklistPopoutId=/^[1-9][0-9]*$/.test(checklistPopoutParam||'')&&Number.isSafeInteger(Number(checklistPopoutParam))?Number(checklistPopoutParam):null;
$('#detail').addEventListener('cancel',e=>{if(checklistPopoutId)e.preventDefault();});
function popoutChecklist(runId){
 const url=new URL(location.href);url.search='';url.hash='';url.searchParams.set('checklistRun',String(runId));
 window.open(url.href,'_blank','popup=yes,width=720,height=900,resizable=yes,scrollbars=yes,noopener,noreferrer');
 toast('Listen åbnes i eget vindue. Tillad pop op-vinduer, hvis den ikke vises.');
}
let checklistItemDraft;
function checklistItemControls(groups){
 const storageKey='occ-checklist-item-draft';
 if(!checklistItemDraft){let saved;try{saved=JSON.parse(sessionStorage.getItem(storageKey));}catch{}
  checklistItemDraft={mode:'open',title:'',runId:'',date:'',checklistCode:'D',...saved,busy:false,uncertain:!!saved?.attempted};
 }
 const d=checklistItemDraft,box=el('details','logEntryForm'),summary=el('summary','','Tilføj punkt');box.open=!!d.title;
 const mode=el('select'),run=el('select'),code=el('select'),date=el('input'),title=el('input'),status=el('p','notice');status.setAttribute('role','status');
 for(const [value,text] of [['open','Åben checkliste'],['future','Fremtidig checkliste']]){const o=el('option','',text);o.value=value;mode.append(o);}
 const placeholder=el('option','','Vælg checkliste');placeholder.value='';run.append(placeholder);
 for(const [name,items] of groups){const id=Number(items[0]?.Run?.Id);if(!Number.isSafeInteger(id)||id<1)continue;const o=el('option','',checklistLabel(name));o.value=String(id);run.append(o);}
 for(const value of ['D','CD','N','CN']){const o=el('option','',value);o.value=value;code.append(o);}
 ...
 date.type='date';date.min=tomorrow.toISOString().slice(0,10);title.type='text';title.maxLength=255;title.placeholder='Skriv punktets titel';
 mode.value=d.mode;run.value=d.runId;code.value=d.checklistCode;date.value=d.date||date.min;title.value=d.title;
 if(checklistPopoutId&&!d.attempted)run.value=String(checklistPopoutId);
 const field=(text,input)=>{const label=el('label','field',text);label.append(input);return label;};
 const runField=field('Åben checkliste',run),codeField=field('Checklistetype',code),dateField=field('Dato',date),hint=el('p','muted');
 const persist=()=>{Object.assign(d,{mode:mode.value,runId:run.value,checklistCode:code.value,date:date.value,title:title.value});try{sessionStorage.setItem(storageKey,JSON.stringify(d));}catch{}};
 let configured=false;
 const paint=()=>{
  runField.hidden=mode.value!=='open';codeField.hidden=dateField.hidden=mode.value!=='future';
  hint.textContent=mode.value==='open'?'Punktet tilføjes øverst på den valgte liste. Kun lister med åbne punkter kan vælges.':'Punktet gælder kun den valgte dag og kommer med, når dagens checkliste oprettes.';
  for(const input of [mode,run,code,date,title])input.disabled=d.busy||d.uncertain;
  save.disabled=!configured||d.busy||d.uncertain;verify.hidden=!d.uncertain;
 };
 const verify=button('Jeg har kontrolleret SharePoint','quiet',()=>{if(!confirm('Har du kontrolleret SharePoint og flowhistorikken for dette forsøg? Et nyt forsøg kan ellers give et dobbelt punkt.'))return;d.uncertain=false;d.attempted=false;d.requestId='';d.title='';title.value='';persist();status.textContent='Kontrol afsluttet. Skriv kun punktet igen, hvis det ikke allerede findes.';paint();});
 const save=button('Tilføj punkt','primary',async()=>{
  if(d.busy||d.uncertain||!configured)return;
  if(!title.value.trim()||title.value.length>255){status.textContent='Skriv en titel på 1–255 tegn.';return;}
  if(mode.value==='open'&&!run.value){status.textContent='Vælg en åben checkliste.';return;}
  if(mode.value==='future'&&(!date.value||date.value<=today||!date.checkValidity())){status.textContent='Vælg en fremtidig dato.';return;}
  d.requestId||=crypto.randomUUID();d.attempted=true;d.busy=true;persist();paint();status.textContent='Gemmer i SharePoint…';
  const payload={mode:d.mode,title:d.title.trim(),requestId:d.requestId,...(d.mode==='open'?{runId:Number(d.runId)}:{checklistCode:d.checklistCode,date:d.date})};
  try{
   const result=await api('checklist-item',{method:'POST',body:JSON.stringify(payload)});if(result.ok!==true)throw new Error('Svaret bekræftede ikke oprettelsen.');
   d.title='';title.value='';d.requestId='';d.attempted=false;openChecklistCache=null;
   status.textContent=d.mode==='open'?'Punktet er gemt. Tryk Refresh for at hente den opdaterede liste.':'Punktet er gemt til '+d.date+'. Det vises, når dagens checkliste oprettes.';
  }catch(e){
   d.uncertain=e.uncertain!==false&&e.status!==400&&e.status!==503;
   if(!d.uncertain){d.attempted=false;d.requestId='';}
   status.textContent=d.uncertain?'Resultatet er usikkert. Kontrollér SharePoint og flowhistorikken før et nyt forsøg. Listen kan også være blevet lukket.':e.message;
  }finally{d.busy=false;persist();paint();}
 });
 for(const input of [mode,run,code,date,title])input.addEventListener('input',()=>{persist();paint();});
 box.append(summary,field('Tilføj til',mode),runField,codeField,dateField,field('Titel',title),hint,save,verify,status);paint();
 if(d.uncertain)status.textContent='Et tidligere forsøg skal kontrolleres i SharePoint og flowhistorikken før et nyt forsøg.';
 api('checklist-item').then(r=>{configured=r.configured===true;if(!configured&&!d.uncertain)status.textContent='Oprettelsesflowet er endnu ikke konfigureret på serveren.';paint();}).catch(()=>{if(!d.uncertain)status.textContent='Kunne ikke kontrollere oprettelsesflowet. Åbn panelet igen.';});
 return box;
}
async function openChecklistStandalone(force=false){
 ...
 try{
  ...
  const groups=groupOpenChecklists(r.data.runitems.value).filter(([,items])=>!checklistPopoutId||Number(items[0]?.Run?.Id)===checklistPopoutId);
  const total=groups.reduce((n,[,items])=>n+items.length,0);
  c.replaceChildren(el('h2','','Åbne checklister'),button('Refresh','quiet',()=>openChecklistStandalone(true)),el('p','muted',groups.length+' checklister · '+total+' åbne punkter'));
  if(checklistPopoutId){document.title=groups.length?checklistLabel(groups[0][0])+' · OCC':'Checkliste · OCC';c.querySelector('h2').textContent=groups.length?checklistLabel(groups[0][0]):'Checkliste';c.append(el('p','notice','Selvstændigt vindue · Brug Refresh for at se ændringer fra andre vinduer.'));}
  c.append(el('p','muted','Afkrydsning gemmes i SharePoint med tidspunkt og dit selvangivne navn.'));
  c.append(checklistItemControls(groups));
  ...
  for(const [run,items] of groups){
   ...
   const runId=Number(items[0]?.Run?.Id);
   if(!checklistPopoutId&&Number.isSafeInteger(runId)&&runId>0)section.append(button('↗ Åbn i eget vindue','quiet checklistPopoutButton',()=>popoutChecklist(runId)));
   section.open=!!checklistPopoutId;
   section.append(nameLabel,saveStatus,list);wrap.append(section);
  }
  if(!groups.length)wrap.append(el('p','empty',checklistPopoutId?'Denne liste har ingen åbne punkter i flowets svar. Den kan være færdig eller uden for henteflowets periode.':'Ingen åbne checklister i flowets svar.'));
